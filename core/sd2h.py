...
try:
    ...
    class autocast:
        ...
        def __exit__(self, *args):
            pass
import sys
sys.path.append('/home/shangying/workspace/project/SD2H-Net/Depth-Anything-V2-list3')
...
class SD2H(nn.Module):
    def __init__(self, args):
        ...
        state_dict_dpt = torch.load(f'/home/shangying/workspace/project/SD2H-Net/pretrained/depth_anything_v2_{args.encoder}.pth', map_location='cpu')
    ...
    def infer_mono(self, image1, image2):                                 #单目回归
        height_ori, width_ori = image1.shape[2:]
        resize_image1 = F.interpolate(image1, scale_factor=14 / 16, mode='bilinear', align_corners=True)     #重采样,align_corners插值时是否对齐角落像素
        resize_image2 = F.interpolate(image2, scale_factor=14 / 16, mode='bilinear', align_corners=True)     #缩放至14/16分辨率本质是​​在ViT的刚性patch划分约束下,通过工程化手段实现效率与精度的最优解​​

        patch_h, patch_w = resize_image1.shape[-2] // 14, resize_image1.shape[-1] // 14             #重采样后resize_image1.shape[-2]表示宽,[-1]表示高,分块14*14,输出分块尺寸
        features_left_encoder = self.mono_encoder.get_intermediate_layers(resize_image1, self.intermediate_layer_idx[self.args.encoder], return_class_token=True)     #预训练模型优先使用中间层特征
        features_right_encoder = self.mono_encoder.get_intermediate_layers(resize_image2, self.intermediate_layer_idx[self.args.encoder], return_class_token=True)    #提取左右图像的中间层特征
        depth_mono = self.mono_decoder(features_left_encoder, patch_h, patch_w)          #根据分块尺寸,解码得单目深度
        depth_mono = F.relu(depth_mono)          #确保深度非负
        depth_mono = F.interpolate(depth_mono, size=(height_ori, width_ori), mode='bilinear', align_corners=False)         #上采样恢复尺寸
        features_left_4x, features_left_8x, features_left_16x, features_left_32x = self.feat_decoder(features_left_encoder, patch_h, patch_w)        #解码成多尺度特征
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        image1 = (2 * (image1 / 3100.0) - 1.0).contiguous()             #3100
        image2 = (2 * (image2 / 3100.0) - 1.0).contiguous()
        ...
        disp_mono_4x = F.interpolate(depth_mono, size=size, mode='bilinear', align_corners=False)           #深度图缩放,减少计算量,

        features_left = self.feat_transfer(features_mono_left)
        features_right = self.feat_transfer(features_mono_right)
        stem_2x = self.stem_2(image1)                                       #特征金字塔,适配不同尺度的物体检测
        ...
        gwc_volume = build_gwc_volume(match_left, match_right, self.args.min_disp//4, self.args.max_disp//4, 8)     #分组构建代价体,self.args.max_disp//4,不同视差下的相似度
        ...
        init_disp = disparity_regression(prob, self.args.min_disp//4, self.args.max_disp//4)                            #初始视差回归,概率分布避免视差离散
        ...
        for itr in range(iters):
            disp = disp.detach()
            if itr >= int(1):
                disp_mono_4x = disp_mono_4x.detach()   #detach()操作断开计算图,防止当前视差预测影响后续优化步骤的梯度传播,当迭代次数超过1时,冻结单目预测的视差图disp_mono_4x,避免其参数在后续步骤中被更新
            geo_feat = geo_fn(disp, coords)
            if itr > int(iters-8):
                if itr == int(iters-7):              #立体视差迭代8次后
                    bs, _, _, _ = disp.shape
                    for i in range(bs):
                        with torch.autocast(device_type='cuda', dtype=torch.float32): 
                            scale, shift = compute_scale_shift(disp_mono_4x[i].clone().squeeze(1).to(torch.float32), disp[i].clone().squeeze(1).to(torch.float32))
                        disp_mono_4x[i] = scale * disp_mono_4x[i] + shift                    #计算单目与立体视差的尺度-偏移校正
                
                warped_right_mono = disp_warp(features_right[0], disp_mono_4x.clone().to(features_right[0].dtype))[0]  
                flaw_mono = warped_right_mono - features_left[0]                           #根据视差将右图特征映射到左图坐标系,计算特征差异,单目残差流

                warped_right_stereo = disp_warp(features_right[0], disp.clone().to(features_right[0].dtype))[0]  #双目残差流
                flaw_stereo = warped_right_stereo - features_left[0] 
                geo_feat_mono = geo_fn(disp_mono_4x, coords)

            if itr <= int(iters-8):           
                net_list, mask_feat_4, delta_disp = self.update_block(net_list, inp_list, geo_feat, disp, iter16=self.args.n_gru_layers==3, iter08=self.args.n_gru_layers>=2)   #前80%迭代,纯立体优化
            else:
                net_list, mask_feat_4, delta_disp = self.update_block_mix_stereo(net_list, inp_list, flaw_stereo, disp, geo_feat, flaw_mono, disp_mono_4x, geo_feat_mono, iter16=self.args.n_gru_layers==3, iter08=self.args.n_gru_layers>=2)   #后20%迭代,单目与立体联合优化
                ...
                disp_mono_4x = disp_mono_4x + delta_disp_mono                                        #视差修正
                disp_mono_4x_up = self.upsample_disp(disp_mono_4x, mask_feat_4_mono, stem_2x)          #上采样
                disp_preds.append(disp_mono_4x_up)                                               #记录中间结果
